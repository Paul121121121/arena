// nav.js
// A ground-level navmesh over the map: which square metres you can stand on,
// and how to get from one to another. Used by the bots to move around, and by
// the tests to check the map is actually traversable.
//
// Loaded by both Node and the browser.

(function (exports, Shared) {
  'use strict';

  const { MAP, CONFIG } = Shared;
...
// A 1-metre grid. At 2m a thin wall could sit entirely between two cell
  ...
  function hasLineOfSight(from, to) {
    ...
    const w = Shared.rayWall(ex, ey, ez, dx / dist, dy / dist, dz / dist, dist + 1);
    return w.t >= dist - 0.1;
  }
  ...
  function stageDuel(shooterPos, targetPos, opts) {
    ...
    const slack = o.slack === undefined ? 3.0 : o.slack;
  
    const candidates = allWalkableCells()
      // The open flank lanes, and clear of the bases at either end. A cell
      // inside a base building counts as walkable but every sightline into it
      // threads a doorway, which is no good for staging a fair fight.
      .filter(c => Math.abs(c.x) >= 28 && Math.abs(c.z) <= 26)
  ...
  exports.walkableCell = walkableCell;
  exports.nearestWalkable = nearestWalkable;
  exports.findPath = findPath;
  exports.smoothPath = smoothPath;
  exports.straightWalkClear = straightWalkClear;
  exports.reachableFrom = reachableFrom;
  exports.allWalkableCells = allWalkableCells;
  exports.hasLineOfSight = hasLineOfSight;
  exports.robustLineOfSight = robustLineOfSight;
  exports.findFiringPosition = findFiringPosition;
  exports.stageDuel = stageDuel;

})(
  typeof module !== 'undefined' && module.exports ? module.exports : (window.Nav = {}),
